...
const double PlaneModel::tail_distance = 2.5; // tail's aerodynamic center from cg
...
const double PlaneModel::max_throttle = 10000.0;
...
void PlaneModel::update(double dt, double delta_elevator, double delta_thrust) {
    ...
    double CL_wing = Cl_aoa_wing * angle_of_attack * (pi / 180.0); //lift coefficient at the wing
    double CL_tail = Cl_aoa_tail * angle_of_attack * (pi / 180.0) + 0.2 * (delta_elevator / delta_max_elevator); //lift coefficient at the tail
